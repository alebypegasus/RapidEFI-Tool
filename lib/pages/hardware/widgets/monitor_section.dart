    ...
    final lines = safeMap(rawInfo['Monitor']).entries.expand((entry) {
      ...
      return [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          HardwareLine([
            'Model: ${entry.key}',
            'Port: ${safeStr(m['Connector Type'])}',
            'Resolution: ${safeStr(m['Resolution'])} @ ${safeStr(m['CurrentRefreshRate'])} Hz',
            safeStr(m['Size']).isEmpty ? '' : '(${safeStr(m['Size'])}")',
          ]),
          Wrap(
              spacing: 12,
              runSpacing: 2,
              ...
              children: [
                SelectableText('Connected GPU: $connectedGpuDisplayName',
